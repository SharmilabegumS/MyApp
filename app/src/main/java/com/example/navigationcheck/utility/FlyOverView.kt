...
public class FlyOverView {
    ...
    constructor(
        parent: View,
        viewToFly: View, @Px finalX: Int, @Px finalY: Int,
        duration: Int,
        listener: OnFlyOverFinishedListener
    ) {
    ...
    fun setupFlyOver(
        ...
        if (LOG_ON) Log.v(
            TAG,
            "FlyOverView, item " + viewToFly + ", finals " + finalX + ", " + finalY + ", sources " + sourceX + ", " + sourceY + " duration " + duration
        );
        ...
        mFlyoverAnimation = ObjectAnimator.ofPropertyValuesHolder(
            ...
            PropertyValuesHolder.ofFloat(
                ...
                1f,
                1f
            ) // not to 0 so we see the end of the effect in other properties
        );
